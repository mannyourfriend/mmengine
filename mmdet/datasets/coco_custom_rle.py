...
class COCOCustomDataset(BaseDetDataset):
	...
	def parse_data_info(self, raw_data_info: dict) -> Union[dict, List[dict]]:
		...
		for i, ann in enumerate(ann_info):
			...
			if seg:
				# COCO API can give you either:
				#  - a list of polygons (list of lists of coords), or
				#  - an RLE dict: {'counts': [...], 'size': [h, w]}
				if isinstance(seg, dict) and 'counts' in seg:
					# Already proper RLE
					rle = seg
				else:
					# Polygon → RLE
					# This returns a list of RLEs (one per poly), so merge them:
